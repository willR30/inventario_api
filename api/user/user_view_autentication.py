...
def register_randon_user_with_email_and_business(request):
    if request.method == 'POST':
        ...
        try:
            with transaction.atomic():  # <-- Garantiza que todo sea transaccional
                # Create the user
                user_serializer = UserSerializer(data=user_data)
                if not user_serializer.is_valid():
                    return Response(
                        {"error": "Failed to create the user", "user_errors": user_serializer.errors},
                        status=status.HTTP_400_BAD_REQUEST
                    )
                user = user_serializer.save()
                ...
                try:
                    ...
                    # Si el correo falla, lanzamos excepción para que haga rollback
                    print("❌ Error enviando correo:", str(e))
                    traceback.print_exc()
                    raise Exception("Email sending failed. Transaction rolled back.")
            ...
            print("❌ Error general en la transacción:", str(e))
            traceback.print_exc()
            return Response({"error": "Transaction failed", "details": str(e)},
                            status=status.HTTP_400_BAD_REQUEST)
